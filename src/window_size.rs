...
pub fn init_window_size(mut commands: Commands, winit_windows: Res<WinitWindows>, windows: Res<Windows>) {
    commands.insert_resource(get_window_size(&winit_windows, &windows));
}

pub fn update_window_size(winit_windows: Res<WinitWindows>, windows: Res<Windows>, mut window_size: ResMut<WindowSize>) {
